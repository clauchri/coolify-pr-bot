...
// src/coolify.ts
var COOLIFY_URL = process.env.COOLIFY_URL;
var COOLIFY_TOKEN = process.env.COOLIFY_TOKEN;
...
async function createDatabase(prNumber) {
...
async function waitForDatabase(dbUuid, maxWaitMs = 60000) {
...
async function getDatabaseInternalUrl(dbUuid, dbName, dbPassword) {
  const db = await coolifyFetch(`/databases/${dbUuid}`);
  const host = db.name || `pr-db-${dbUuid}`;
  return `postgresql://postgres:${dbPassword}@${host}:5432/${dbName}`;
}
async function setPreviewEnv(appUuid, key, value) {
...
async function triggerDeploy(appUuid, prNumber) {
  const result = await coolifyFetch(`/deploy?uuid=${appUuid}&pr=${prNumber}`);
  console.log(`[PR #${prNumber}] Triggered deploy: ${JSON.stringify(result)}`);
  return result;
}
async function deleteDatabase(dbUuid) {
...
async function listProjectDatabases() {
  const projectUuid = process.env.COOLIFY_PROJECT_UUID;
  const environmentName = process.env.COOLIFY_ENVIRONMENT_NAME || "production";
  const env = await coolifyFetch(`/projects/${projectUuid}/${environmentName}`);
  return (env.databases || []).map((db) => ({
    uuid: db.uuid,
    name: db.name,
    description: db.description
  }));
}
async function findDatabaseForPR(prNumber) {
  const databases = await listProjectDatabases();
  const match2 = databases.find((db) => db.name === `pr-${prNumber}-db`);
  return match2?.uuid ?? null;
}

// src/index.ts
var app = new Hono2;
var GITHUB_WEBHOOK_SECRET = process.env.GITHUB_WEBHOOK_SECRET;
var APP_UUID = process.env.COOLIFY_APP_UUID;
var HANDLED_ACTIONS = [
  "opened",
  "reopened",
  "synchronize",
...
app.post("/webhook/github", async (c) => {
  ...
  console.log(`[PR #${pr.number}] ${pr.action} \u2014 "${pr.title}" by ${pr.sender}`);
  const promise = handlePREvent(pr);
  promise.catch((err) => console.error(`[PR #${pr.number}] Error handling ${pr.action}:`, err));
...
async function handlePROpened(prNumber) {
...
async function handlePRSync(prNumber) {
  const dbUuid = await findDatabaseForPR(prNumber);
  if (!dbUuid) {
    console.log(`[PR #${prNumber}] No database found on sync, creating one...`);
...
async function handlePRClosed(prNumber) {
